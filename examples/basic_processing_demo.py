...
def META_metrics_test(dataset_name):
    ...
    sens_indexs = [dataset_orig_train.feature_names.index(feature) for feature in dataset_orig_train.protected_attribute_names]
    print("-->sens_indexs", sens_indexs)
    ...
    # dataset_bias_test.labels = orig_model.predict(dataset_bias_test.features)
    #
    ...
    multi_group_metrics = metric_test_multivariate(dataset=classified_dataset,   # classified_dataset
                                                   model=orig_model,
                                                   thresh_arr=thresh_arr,
                                                   unprivileged_groups=unprivileged_groups,
                                                   privileged_groups=privileged_groups
    ...
    indexs = [dataset_orig_train.feature_names.index(protected_name) for protected_name in protected_attributes]
    print("-->indexs", indexs)
    train = np.delete(dataset_orig_train.features, indexs, axis=1)
    # test = dataset_orig_test.copy()
    test = np.delete(dataset_orig_test.features, indexs, axis=1)

    trans_model = MLPClassifier(solver='adam', activation='identity', max_iter=500, alpha=1e-5,
                               hidden_layer_sizes=(64, 32, 16, 8, 4),
                               random_state=1, verbose=True)
    trans_model.fit(train, dataset_orig_train.labels.ravel())

    y_test = dataset_orig_test.labels.ravel()
    y_trans_pred = trans_model.predict(test)
    print("-->prediction accuracy on test data", accuracy_score(list(y_test), list(y_trans_pred)))

    classi_test = np.delete(classified_dataset.features, indexs, axis=1)
    classi_trains_pred = trans_model.predict(classi_test)
    ...
    for sens_ind in sens_inds:
        ...
        index = dataset_orig_train.feature_names.index(sens_attr)
        ...
        train = np.delete(dataset_orig_train.features, [index], axis=1)
        # test = dataset_orig_test.copy()
        test = np.delete(dataset_orig_test.features, [index], axis=1)
        ...
        classi_test = np.delete(classified_dataset.features, [index], axis=1)
        classi_trains_pred = trans_model.predict(classi_test)
