...
class AdversarialDebiasing(Transformer):
    ...
    def __init__(self,
        ...
        try:
            self.protected_attribute_name = [list(self.unprivileged_groups[i].keys())[0] for i in
                                             range(0, len(self.unprivileged_groups))]
        except:
            self.protected_attribute_name = list(self.unprivileged_groups[0].keys())

        # if len(self.protected_attribute_name) == 1:
        #     self.protected_attribute_name = self.protected_attribute_name[0]
        print("-->self.protected_attribute_name", self.protected_attribute_name)
    ...
    def fit(self, dataset):
        ...
        with tf.variable_scope(self.scope_name):
            ...
            self.protected_attributes_ph = tf.placeholder(tf.float32)
            ...
            for epoch in range(self.num_epochs):
                ...
                for i in range(num_train_samples//self.batch_size):
                    ...
                    try:
                        batch_protected_attributes = np.reshape(dataset.protected_attributes[batch_ids][:,
                                                 dataset.protected_attribute_names.index(self.protected_attribute_name[0])], [-1,1])

                    except:
                        pro_attributes = []
                        for attribute in self.protected_attribute_name:
                            # index = dataset.feature_names.index(attribute)
                            pro_attributes.append(np.reshape(dataset.protected_attributes[batch_ids][:,
                                                 dataset.protected_attribute_names.index(attribute)], [-1,1]))
                        batch_protected_attributes = np.append(pro_attributes[0], pro_attributes[1], axis=1)
            ...
            try:
                batch_protected_attributes = np.reshape(dataset.protected_attributes[batch_ids][:,
                                                        dataset.protected_attribute_names.index(
                                                            self.protected_attribute_name[0])], [-1, 1])
    ...
    def predict_one_input(self, dataset):
        """Obtain the predictions for the provided dataset using the fair
        classifier learned.
        ...
        while samples_covered < num_test_samples:
            ...
            labels = np.reshape(dataset.labels, [-1, 1])
            # print("-->features", features)
            # print("-->self.protected_attribute_name", self.protected_attribute_name)
            try:
                protected_attributes = np.reshape(features[:,dataset.protected_attribute_names.index(
                                                            self.protected_attribute_name[0])], [-1, 1])
            except:
                pro_attributes = []
                for attribute in self.protected_attribute_name:
                    # index = dataset.feature_names.index(attribute)
                    pro_attributes.append(np.reshape(np.array(features)[:,
                                                     dataset.feature_names.index(attribute)], [-1, 1]))
                try:
                    protected_attributes = np.append(pro_attributes[0], pro_attributes[1], axis=1)
                except:
                    protected_attributes = np.array(pro_attributes)
                # print("-->protected_attributes", protected_attributes)

            batch_feed_dict = {self.features_ph: features,
                               self.true_labels_ph: labels,
                               self.protected_attributes_ph: protected_attributes,
                               self.keep_prob: 1.0}

            pred_labels += self.sess.run(self.pred_labels, feed_dict=batch_feed_dict)[:,0].tolist()
            samples_covered += len(features)
