...
class NewInputClassificationMetric(BinaryLabelDatasetMetric):

    def __init__(self, dataset, classified_dataset,
                 unprivileged_groups=None, privileged_groups=None):
        """
        Args:
            dataset (BinaryLabelDataset): Generated new inputs (without ground-truth labels)
            predicted_labels (BinaryLabelDataset): Predicted Labels of new inputs
        ...
        super(NewInputClassificationMetric, self).__init__(dataset, classified_dataset,
            unprivileged_groups=unprivileged_groups,
            privileged_groups=privileged_groups)

        self.sampling_threshold = 500
    ...
    def margin_of_error(self, inputs, input_labels):
        # num_true_positive = self.num_true_positives(privileged=privileged)
        # num_false_postive = self.num_false_positives(privileged=privileged)
        # num_inputs = self.num_instances(privileged=privileged)
        # pro_positive = float(num_true_positive+num_false_postive)/num_inputs
        num_positive = input_labels.count(1)
        num_inputs = len(inputs)
        pro_positive = float(num_positive)/float(num_inputs)
        margin_error = self.z_value*math.sqrt((pro_positive*(1-pro_positive))/num_inputs)
        return margin_error

    def statistical_parity_difference_with_error(self, privileged=None):
        # index = self.sampling_threshold
        # inputs = self.dataset[:index]
        # print("length of inputs", len(inputs))
        # input_labels = self.predicted_labels[:index]
        # error = self.margin_of_error(inputs, input_labels)
