...
def compute_boolean_conditioning_vector(X, feature_names, condition=None):
    ...
    # print("-->condition", condition)  # [{'race': 1.0}, {'sex': 1.0}]
    # for group in condition:
    #     group_cond = np.ones(X.shape[0], dtype=bool)
    #     print("-->group", group)
    #     for name, val in group.items():
    #         print("-->name", name)  # race, sex
    #         print("-->val", val)  # 1, 1
    #         index = feature_names.index(name)
    #         group_cond = np.logical_and(group_cond, X[:, index] == val)
    #         print("-->group_cond:", list(group_cond[:20]))
    #     if "0.0" in str(condition):
    #     # if any(["0.0" in one_cond for one_cond in condition]):
    #         overall_cond = np.logical_or(overall_cond, group_cond)
    #     else:
    #         overall_cond = np.logical_and(overall_cond, group_cond)
    #     # # overall_cond = np.logical_or(overall_cond, group_cond)
    #     # overall_cond = np.logical_and(overall_cond, group_cond)
    #     print("-->overall_cond", list(overall_cond[:20]))

    if condition is None:
        return np.ones(X.shape[0], dtype=bool)

    overall_cond = np.ones(X.shape[0], dtype=bool)
    for group in condition:
        group_cond = np.ones(X.shape[0], dtype=bool)
        for name, val in group.items():
            index = feature_names.index(name)
