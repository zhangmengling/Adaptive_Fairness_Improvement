...
class StructuredDataset(Dataset):
    ...
    def align_datasets(self, other):
        ...
        print("-->other.feature_names", other.feature_names)
        print(self.protected_attribute_names)
        print(other.protected_attribute_names)
        ...
        # changed self.feature_names-->other.feature_names
    ...
    def convert_to_dataframe(self, de_dummy_code=False, sep='=',
        ...
        try:
            ...
            self.labels = np.array([[1]]*len(self.features))
            print(len(self.features), len(self.features[0]))
            print(len(self.labels))
            print(len(self.feature_names + self.label_names))
            print(len(self.instance_names))
            df = pd.DataFrame(np.hstack((self.features, self.labels)),
                              columns=self.feature_names + self.label_names,
                              index=self.instance_names)
        print(len(self.protected_attribute_names))
        print(len(self.protected_attributes))
