...
class CausalImpl():
    ...
    #
    def solve(self, model, assertion, term, display=None):
        ...
        if 'all_datapath' in spec:
            self.all_datapath = spec['all_datapath']

        if 'term_datapath' in spec:
            self.term_datapath = spec['term_datapath']
        ...
        if self.solve_option == 'solve_fairness':
            self.solve_fairness(model, assertion, term)
        else:
    ...
    #   solve for fairness improvement
    #
    def solve_fairness(self, model, assertion, term, display=None):
        overall_starttime = time.time()
        self.model = model
        self.assertion = assertion
        self.display = display
        ...
        term_index_file = self.term_datapath + term[0] + "_index.txt"
        f = open(term_index_file)
        term_indexs = f.readlines().tolist()

        pathX = self.datapath + '/'
        pathY = self.datapath + '/labels.txt'
        ys = np.array(ast.literal_eval(read(pathY)))
        length = ts.shape[0]
        y_sum = 0.0

        for index in term_indexs:
            x0_file = pathX + 'data' + str(index) + '.txt'
        ...
        if not self.plot:
            if self.object == "neuron":
                ...
                plt.savefig(self.resultpath + '/' + self.object + 'high_causal_effect' + ".png")
                print("-->save path", self.resultpath + '/' + 'high_causal_effect' + ".png")
